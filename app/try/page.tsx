...
import { Shirt,Briefcase,GraduationCap,Heart,PartyPopper,MoreHorizontal,
  Wind,Waves,Zap,Minimize2,Crown,Sun,Cloud,CloudRain,Snowflake,Flame, ArrowLeft,Loader2,Sparkles
} from "lucide-react";
  ...
  const [error,    setError   ] = useState("");
    ...
    try{
      ...
      sessionStorage.setItem(
        "outfit_result",
        JSON.stringify({outfit: data.outfit, occasion, style, weather})
      );
      router.push("/result");
    }
    catch{
            ...
            <div className="flex flex-wrap gap-3">
              {weathers.map((w) => (
                <Chip
                  ...
                  onClick={() =>
                    setWeather((prev) => (prev === w.value ? null : w.value))
                  }
                />
              ))}
            </div>
